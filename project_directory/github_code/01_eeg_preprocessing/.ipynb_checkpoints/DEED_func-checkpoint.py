def epoching(args):
    ...
    Returns
    -------
    epoched_data : list of array, [array(1,6,times,) x number of rems]
        Epoched EEG data. 
    ch_names : list of str
        EEG channel names.
    times : list of array, [array(times,) x number of rems]
        EEG time points.
    ...
    for filename in os.listdir(os.path.join(args.project_dir, eeg_dir)):  
        if filename == args.PSG_number:
            break
        else:
            new_filename = filename[5:8]
            old_file_path = os.path.join(args.project_dir, eeg_dir, filename)
            new_file_path = os.path.join(args.project_dir, eeg_dir, new_filename)
    ...
    for index, row in f2.iterrows():
        psg_number = str(row['PSG_number'])[5:8]
        if psg_number == args.PSG_number:
...
def mvnn(args, epoched_data):
    ...
    Returns
    -------
    whitened_data : list of array, [array(6,times,) x number of rems]
        Whitened EEG data. 
...
def save_prepr(args, whitened_data, ch_names, times):
    ...
    import os
    ### Save the data ###
    save_dir = os.path.join(args.project_dir, 'eeg_dataset', 'dream_data', 'preprocessed_data', 'DEED');
    # Create the directory if it doesn't exist #
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)

    for i, (data, t) in enumerate(zip(whitened_data, times)):
        ...
        folder_name = args.PSG_number
        save_folder = os.path.join(save_dir, folder_name)
        if not os.path.isdir(save_folder):
            os.makedirs(save_folder)
        file_name = folder_name + '_' + str(i+1)
