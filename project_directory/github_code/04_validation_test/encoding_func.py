def train_model_spatial(args):
    ...
    # THINGS_EEG1: (16540,17) / Zhang_Wamsley: (16540,16)
    ...
    # THINGS_EEG1: (images,17) / Zhang_Wamsley: (images,16)
...
def test_model_ZW_temporal(args, pred_eeg_data_test, test_subj):
    ...
    args : Namespace
        Input arguments.
    pred_eeg_data_test: array with shape (images/dreams, channels, times)
        The predicted EEG data.
    test_subj : int
        Used test subject.
    ...
    eeg_data_test = np.load(os.path.join(eeg_test_dir, eeg_test_list[test_subj]),
                            allow_pickle=True).item()
    print(eeg_test_list[test_subj])
    eeg_data_test = eeg_data_test['preprocessed_eeg_data']
    # Crop the test EEG data of last 20s (16,2000)
    eeg_data_test = eeg_data_test[:,-2000:]
    # Get the number of time points
    num_t = eeg_data_test.shape[1]
    
    ### Select the target dream predicted EEG (8, 16 x 15) ###
    pred_eeg_data_test = pred_eeg_data_test[int(test_subj*8):int(test_subj*8+8)]

    ### Test the encoding model ###
    encoding_accuracy = np.zeros((8,int(num_t-15)))
    for img in range(8):
        for t in range(num_t):
            encoding_accuracy[img,t] = corr(pred_eeg_data_test[img], 
                                            eeg_data_test[:,int(t*15):int(t*15+15).ravel()])[0]
